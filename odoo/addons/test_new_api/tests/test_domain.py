...
class test_domain(common.TransactionCase):
    ...
    def test_00_test_bool_undefined(self):
        ...
        self.env.ref('test_new_api.bool_3').write(
            {'x_bool_new_undefined': True})
        self.env.ref('test_new_api.bool_4').write(
            {'x_bool_new_undefined': False})
        ...
        for f in ['bool_true', 'bool_false', 'bool_undefined', 'x_bool_new_undefined']:
            ...
            self.assertEqual(neq_1 + neq_2, all_bool,
                             'not True + not False != all')
