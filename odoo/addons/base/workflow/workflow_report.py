...
def graph_get(cr, graph, wkf_ids, nested, workitem, witm_trans, processed_subflows):
    import pydot
    cr.execute('select * from wkf_activity where wkf_id in (' +
    ...
    for n in nodes:
        ...
        if n['subflow_id'] and nested and n['subflow_id'] not in processed_subflows:
            ...
            processed_subflows.add(n['subflow_id'])
            cr.execute('select * from wkf where id=%s', (n['subflow_id'],))
            wkfinfo = cr.dictfetchone()
            graph2 = pydot.Cluster('subflow' + str(n['subflow_id']), fontsize='12',
                                   label="\"Subflow: %s\\nOSV: %s\"" % (n['name'], wkfinfo['osv']))
            (s1, s2) = graph_get(cr, graph2, [
                n['subflow_id']], True, workitem, witm_trans, processed_subflows)
    ...
    cr.execute(
        'select * from wkf_transition where act_from IN %s ORDER BY sequence,id', (node_ids,))
    ...
    for t in transitions:
        ...
        activity_to = actto[t['act_to']][1].get(
            t['signal'], actto[t['act_to']][0])
        graph.add_edge(pydot.Edge(str(activity_from), str(
            activity_to), fontsize='10', **args))

    cr.execute('select * from wkf_activity where flow_start=True and wkf_id in (' +
               ','.join(['%s'] * len(wkf_ids)) + ')', wkf_ids)
    start = cr.fetchone()[0]
    cr.execute("select 'subflow.'||name,id from wkf_activity where flow_stop=True and wkf_id in (" +
               ','.join(['%s'] * len(wkf_ids)) + ')', wkf_ids)
    stop = cr.fetchall()
    if stop:
...
def graph_instance_get(cr, graph, inst_id, nested=False):
    ...
    def workitem_get(instance):
        cr.execute(
            'select act_id,count(*) from wkf_workitem where inst_id=%s group by act_id', (instance,))
        workitems = dict(cr.fetchall())

        cr.execute(
            'select subflow_id from wkf_workitem where inst_id=%s', (instance,))
    ...
    def witm_get(instance):
        cr.execute(
            "select trans_id from wkf_witm_trans where inst_id=%s", (instance,))
        return set(t[0] for t in cr.fetchall())

    processed_subflows = set()
    graph_get(cr, graph, [x[0] for x in inst], nested, workitem_get(
        inst_id), witm_get(inst_id), processed_subflows)
...
showpage'''
                ...
                cr.execute(
                    'select i.id from wkf_instance i left join wkf w on (i.wkf_id=w.id) where res_id=%s and osv=%s', (data['id'], data['model']))
...
showpage'''
                    ...
                    for inst_id in inst_ids:
                        inst_id = inst_id[0]
                        graph_instance_get(
                            cr, graph, inst_id, data.get('nested', False))
