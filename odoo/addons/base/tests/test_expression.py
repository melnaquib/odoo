...
class TestExpression(TransactionCase):
    ...
    def test_00_in_not_in_m2m(self):
        # Create 4 partners with no category, or one or two categories (out of
        # two categories).
        ...
        ab = partners.create({'name': 'test_expression_partner_AB', 'category_id': [
                             (6, 0, [cat_a.id, cat_b.id])]})
        ...
        self.assertEqual(
            a + ab, with_a, "Search for category_id in cat_a failed.")

        with_b = partners.search([('category_id', 'in', [cat_b.id])])
        self.assertEqual(
            b + ab, with_b, "Search for category_id in cat_b failed.")

        # Partners with the category A or the category B.
        with_a_or_b = partners.search(
            [('category_id', 'in', [cat_a.id, cat_b.id])])
        self.assertEqual(a + b + ab, with_a_or_b,
                         "Search for category_id contains cat_a or cat_b failed.")

        # Show that `contains list` is really `contains element or contains
        # element`.
        ...
        self.assertFalse(without_a_or_b & (
            a + b + ab), "Search for category_id doesn't contain cat_a or cat_b failed (1).")
        self.assertTrue(
            c in without_a_or_b, "Search for category_id doesn't contain cat_a or cat_b failed (2).")

        # Show that `doesn't contain list` is really `doesn't contain element
        # and doesn't contain element`.
        without_a_and_without_b = partners.search(
            [('category_id', 'not in', [cat_a.id]), ('category_id', 'not in', [cat_b.id])])
        self.assertFalse(without_a_and_without_b & (
            a + b + ab), "Search for category_id doesn't contain cat_a and cat_b failed (1).")
    ...
    def test_05_not_str_m2m(self):
        ...
        for name, cat_ids in partners_config.items():
            pids[name] = partners.create(
                {'name': name, 'category_id': [(6, 0, cat_ids)]}).id

        base_domain = [('id', 'in', list(pids.values()))]

        def test(op, value, expected):
            found_ids = partners.search(
                base_domain + [('category_id', op, value)]).ids
    ...
    def test_10_hierarchy_in_m2m(self):
        ...
        cats = Category.search(
            [('id', 'parent_of', (categ_root + categ_0).ids)])
        self.assertEqual(len(cats), 2)
    ...
    def test_15_m2o(self):
        ...
        for i in range(4):
            Partner.create({'name': 'P of Acme %s' % i,
        ...
        self.assertTrue(len(partners) >= 4,
        ...
        self.assertTrue(len(
            partners) >= 4, "We should have at least 4 partners not related to company #1")

        # check that many2one will exclude the correct records with a list and
        # False
        partners = Partner.search(['|', ('company_id', 'not in', [1]),
                                   ('company_id', '=', False)])
        self.assertTrue(len(
            partners) >= 8, "We should have at least 8 partners not related to company #1")
    ...
    def test_15_equivalent_one2many_1(self):
        ...
        self.assertEqual([p1], Partner.search(
            [('user_ids', 'in', u1a)]).ids, "o2m IN accept single int on right side")
        self.assertEqual([p1], Partner.search(
            [('user_ids', '=', 'Dédé Boitaclou')]).ids, "o2m NOT IN matches none on the right side")
        self.assertEqual([], Partner.search(
            [('user_ids', 'in', [10000])]).ids, "o2m NOT IN matches none on the right side")
        self.assertEqual([p1, p2], Partner.search(
            [('user_ids', 'in', [u1a, u2])]).ids, "o2m IN matches any on the right side")
        all_ids = Partner.search([]).ids
        self.assertEqual(set(all_ids) - set([p1]), set(Partner.search(
            [('user_ids', 'not in', u1a)]).ids), "o2m NOT IN matches none on the right side")
        self.assertEqual(set(all_ids) - set([p1]), set(Partner.search(
            [('user_ids', '!=', 'Dédé Boitaclou')]).ids), "o2m NOT IN matches none on the right side")
        self.assertEqual(set(all_ids) - set([p1, p2]), set(Partner.search(
            [('user_ids', 'not in', [u1b, u2])]).ids), "o2m NOT IN matches none on the right side")
    ...
    def test_15_equivalent_one2many_2(self):
        ...
        currency = Currency.create(
            {'name': 'ZZZ', 'symbol': 'ZZZ', 'rounding': 1.0})
        ...
        # back at the currency)
        currency_rate1 = CurrencyRate.search(
            [('name', 'not like', 'probably_unexisting_name')])
        currency_rate2 = CurrencyRate.search(
            [('id', 'not in', [non_currency_id])])
        ...
        res_9 = Currency.search(
            [('rate_ids', 'like', 'probably_unexisting_name')])
        self.assertFalse(res_9)
        # Currency.search([('rate_ids', 'unexisting_op',
        # 'probably_unexisting_name')]) # TODO expected exception

        # get the currencies referenced by some currency rates using a weird
        # negative domain
        res_10 = Currency.search(
            [('rate_ids', 'not like', 'probably_unexisting_name')])
    ...
    def test_20_expression_parse(self):
        ...
        self.assertEqual(users, a + b1 + b2,
                         'searching through inheritance failed')
        ...
        # Special =? operator mean "is equal if right is set, otherwise always
        # True"
        users = Users.search(
            [('name', 'like', 'test'), ('parent_id', '=?', False)])
    ...
    def test_30_normalize_domain(self):
        ...
        self.assertEqual(norm_domain, expression.normalize_domain(
            domain), "Normalized domains should be left untouched")
        domain = [('x', 'in', ['y', 'z']), ('a.v', '=', 'e'), '|', '|', ('a',
                                                                         '=', 'b'), '!', ('c', '>', 'd'), ('e', '!=', 'f'), ('g', '=', 'h')]
        norm_domain = ['&', '&', '&'] + domain
        self.assertEqual(norm_domain, expression.normalize_domain(
            domain), "Non-normalized domains should be properly normalized")
    ...
    def test_accent(self):
        ...
        self.assertNotIn(helene, Company.search(
            [('name', 'not ilike', 'Helene')]))
        self.assertNotIn(helene, Company.search(
            [('name', 'not ilike', 'hélène')]))

    def test_like_wildcards(self):
        # check that =like/=ilike expressions are working on an untranslated
        # field
        Partner = self.env['res.partner']
        ...
        self.assertTrue(len(partners) >= 1,
                        "Must match one partner (China Export)")
        ...
        self.assertTrue(len(
            countries) == 3, "Must match only countries with names starting with Z (currently 3)")
    ...
    def test_long_table_alias(self):
        ...
        self.patch_order(
            'res.partner', 'commercial_partner_id,company_id,name')
    ...
    def test_lp1071710(self):
        ...
        not_be = Country.with_context(lang='fr_FR').search(
            [('name', '!=', 'Belgique')])
...
class TestAutoJoin(TransactionCase):
    ...
    def setUp(self):
        ...
        # Mock BaseModel._where_calc(), to be able to proceed to some tests
        # about generated expression
        self._reinit_mock()
    ...
    def test_auto_join(self):
        ...
        expected = "%s::text like %s" % (
            unaccent('"res_partner_bank"."sanitized_acc_number"'), unaccent('%s'))
        ...
        expected = "%s::text like %s" % (
            unaccent('"res_partner__bank_ids"."sanitized_acc_number"'), unaccent('%s'))
        self.assertIn(expected, sql_query[1],
        ...
        partners = partner_obj.search(
            [('child_ids.bank_ids.id', 'in', bank_ids)])
        # Test result
        self.assertEqual(partners, p_a + p_b,
        ...
        self._reinit_mock()
        partners = partner_obj.search(
            [('state_id.country_id.code', 'like', name_test)])
        ...
        expected = "%s::text like %s" % (
            unaccent('"res_country"."code"'), unaccent('%s'))
        self.assertIn(expected, sql_query[1],
        ...
        expected = "%s::text like %s" % (
            unaccent('"res_partner__state_id__country_id"."code"'), unaccent('%s'))
        self.assertIn(expected, sql_query[1],
        ...
        patch_auto_join(partner_obj, 'child_ids', True)
        patch_auto_join(partner_obj, 'bank_ids', True)
        patch_domain(partner_obj, 'child_ids', lambda self: [
                     '!', ('name', '=', self._name)])
        ...
        expected = "%s::text like %s" % (unaccent(
            '"res_partner__child_ids__bank_ids"."sanitized_acc_number"'), unaccent('%s'))
        self.assertIn(expected, sql_query[1],
                      "_auto_join on one2many with domains incorrect result")
        ...
        # Do: ('child_ids.state_id.country_id.code', 'like', '..') without
        # _auto_join
        self._reinit_mock()
        partners = partner_obj.search(
        ...
        # Do: ('child_ids.state_id.country_id.code', 'like', '..') with
        # _auto_join
        patch_auto_join(partner_obj, 'child_ids', True)
        patch_auto_join(partner_obj, 'state_id', True)
        patch_auto_join(state_obj, 'country_id', True)
