...
def load_module_graph(cr, graph, status=None, perform_checks=True, skip_modules=None, report=None):
        ...
        try:
            ...
            for filename in _get_files_of_kind(kind):
                ...
                if kind in ('demo', 'demo_xml') or (filename.endswith('.csv') and kind in ('init', 'init_xml')):
                    noupdate = True
                tools.convert_file(cr, module_name, filename,
                                   idref, mode, noupdate, kind, report)
        finally:
            if kind in ('demo', 'test'):
    ...
    for index, package in enumerate(graph, 1):
        ...
        _logger.debug('loading module %s (%d/%d)',
                      module_name, index, module_count)
        ...
        if hasattr(package, 'init') or hasattr(package, 'update') or package.state in ('to install', 'to upgrade'):
            ...
            if has_demo:
                # launch tests only in demo mode, allowing tests to use demo
                # data.
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(
        graph), time.time() - t0, odoo.sql_db.sql_counter - t0_sql)
...
def _check_module_names(cr, module_names):
    ...
    if mod_names:
        ...
        if cr.dictfetchone()['count'] != len(mod_names):
            ...
            _logger.warning('invalid module names, ignored: %s',
...
def load_marked_modules(cr, graph, states, force, progressdict, report, loaded_modules, perform_checks):
    ...
    while True:
        ...
        loaded, processed = load_module_graph(
            cr, graph, progressdict, report=report, skip_modules=loaded_modules, perform_checks=perform_checks)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    try:
        ...
        # STEP 1: LOAD BASE (must be done before module dependencies can be
        # computed for later steps)
        ...
        if update_module:
            ...
            _check_module_names(cr, itertools.chain(
                list(tools.config['init'].keys()), list(tools.config['update'].keys())))

            module_names = [k for k, v in list(
                tools.config['init'].items()) if v]
            if module_names:
            ...
            module_names = [k for k, v in list(
                tools.config['update'].items()) if v]
            if module_names:
                modules = Module.search(
            ...
            cr.execute(
                "update ir_module_module set state=%s where name=%s", ('installed', 'base'))
        ...
        if processed_modules:
            ...
            for (model, name) in cr.fetchall():
                if model in registry and registry[model]._transient:
                    _logger.warning(
                        'The transient model %s (%s) should not have explicit access rules!', model, name)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                Module = env['ir.module.module']
                Module.browse(list(modules_to_remove.values())
                              ).module_uninstall()
