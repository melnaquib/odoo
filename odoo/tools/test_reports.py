...
def try_report(cr, uid, rname, ids, data=None, context=None, our_module=None, report_type=None):
    ...
    res = odoo.report.render_report(
        cr, uid, ids, rname_s, data, context=context)
    ...
    _logger.debug("Have a %s report for %s, will examine it",
                  res_format, rname)
    if res_format == 'pdf':
        if res_data[:5] != '%PDF-':
            raise ValueError(
        ...
        try:
            ...
            proc = Popen(['pdftotext', '-enc', 'UTF-8', '-nopgbrk',
                          rfname, '-'], shell=False, stdout=PIPE)
        ...
        if res_text is not False:
            for line in res_text.split('\n'):
                if ('[[' in line) or ('[ [' in line):
                    _logger.error(
                        "Report %s may have bad expression near: \"%s\".", rname, line[80:])
...
def try_report_action(cr, uid, action_id, active_model=None, active_ids=None,
    ...
    def _exec_action(action, datas, env):
        ...
        if isinstance(action, bool) or 'type' not in action:
            return
        # Updating the context : Adding the context of action in order to use
        # it on Views called from buttons
        context = dict(env.context)
        if datas.get('id', False):
            context.update({'active_id': datas.get('id', False), 'active_ids': datas.get(
                'ids', []), 'active_model': datas.get('model', False)})
        ...
        if action['type'] in ['ir.actions.act_window', 'ir.actions.submenu']:
            ...
            _logger.debug('Buttons are: %s', ', '.join(
            ...
            while buttons and not res:
                ...
                if not b['type']:
                    log_test(
                        "the \"%s\" button has no type, cannot use it", b['string'])
                    continue
                if b['type'] == 'object':
                    ...
                    if not func:
                        _logger.error(
                            "The %s model doesn't have a %s attribute!", datas['res_model'], b['name'])
                        continue
                    res = func()
                    break
                else:
            ...
            if 'ids' in datas:
                del datas['ids']
            res = try_report(
                cr, uid, 'report.' + action['report_name'], ids, datas, context, our_module=our_module)
            return res
        else:
    ...
    while action:
        ...
        log_test("it is an %s action at loop #%d",
                 action.get('type', 'unknown'), loop)
