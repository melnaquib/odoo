...
class graph(object):
    ...
    def init_rank(self):
        ...
        while self.tight_tree() < tot_node:
            ...
            for edge in self.edge_wt:
                self.edge_wt[edge] = self.result[edge[1]
                                                 ]['x'] - self.result[edge[0]]['x']
    ...
    def rev_edges(self, tree):
        ...
        for link in self.links:
            src = link[0]
            des = link[1]
            edge_len = self.partial_order[des]['level'] - \
    ...
    def tree_order(self, node, last=0):
        ...
        for i, child in enumerate(first_half):
            ...
            if self.transitions.get(child, False):
                if last:
                    self.result[child]['y'] = last + \
                        len(self.transitions[child]) / 2 + 1
        ...
        if rest:
            ...
            if self.transitions.get(mid_node, False):
                if last:
                    self.result[mid_node]['y'] = last + \
                        len(self.transitions[mid_node]) / 2 + 1
                if node != mid_node:
    ...
    def process_order(self):
        ...
        if self.Is_Cyclic:
            ...
            if max_level % 2:
                ...
                self.result[self.start]['y'] = max_level / \
                    2 + self.max_order + (self.max_order and 1)
            ...
            min_order = math.fabs(
                min([x['y'] for x in list(self.result.values())]))
...
#        self.finalize_rank(self.start, 0)
    ...
    def order_in_rank(self):
        ...
        for level in self.levels:
            self.levels[level].sort(lambda x, y: cmp(
                self.result[x]['y'], self.result[y]['y']))
    ...
    def process(self, starting_node):
        ...
        if self.nodes:
            if self.start_nodes:
                # add dummy edges to the nodes which does not have any incoming
                # edges
...
if __name__ == '__main__':
    ...
    nodes = ['project', 'account', 'hr', 'base',
             'product', 'mrp', 'test', 'profile']
    transitions = [
        ('profile', 'mrp'),
        ('mrp', 'project'),
        ('project', 'product'),
    ...
    for name, node in list(node_res.items()):
        ...
        draw.arc((int(node['y'] - radius), int(node['x'] - radius), int(
            node['y'] + radius), int(node['x'] + radius)), 0, 360, (128, 128, 128))
        draw.text((int(node['y']),  int(node['x'])),
                  str(name),  (128, 128, 128))

    for t in transitions:
        draw.line((int(node_res[t[0]]['y']), int(node_res[t[0]]['x']), int(
            node_res[t[1]]['y']), int(node_res[t[1]]['x'])), (128, 128, 128))
