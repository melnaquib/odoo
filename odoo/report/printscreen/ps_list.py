...
class report_printscreen_list(report_int):
    ...
    def create(self, cr, uid, ids, datas, context=None):
        ...
        if self.groupby:
            ...
            def get_groupby_data(groupby=[], domain=[]):
                records = Model.read_group(
                    domain, fields_order, groupby, 0, None)
                for rec in records:
                    rec['__group'] = True
                    rec['__no_leaf'] = self.groupby_no_leaf
                    rec['__grouped_by'] = groupby[0] if (
                        isinstance(groupby, list) and groupby) else groupby
                    for f in fields_order:
                        if f not in rec:
                            ...
                            rec[f] = rec[f][1]
                    rows.append(rec)
                    inner_groupby = (rec.get('__context', {})
                                     ).get('group_by', [])
        ...
        res = self._create_table(
            uid, datas['ids'], result['fields'], fields_order, rows, context, model_desc)
    ...
    def _create_table(self, uid, ids, fields, fields_order, results, context, title=''):
        ...
        _append_node('header-date', str(rml_obj.formatLang(time.strftime("%Y-%m-%d"),
                                                           date=True)) + ' ' + str(time.strftime("%H:%M")))
        l = []
        t = 0
        ...
        for line in results:
            ...
            for f in fields_order:
                ...
                if fields[f]['type'] == 'datetime' and line[f]:
                    new_d1 = line[f]
                    if not line.get('__group'):
                        format = str(locale.nl_langinfo(locale.D_FMT).replace(
                            '%y', '%Y')) + ' ' + str(locale.nl_langinfo(locale.T_FMT))
                ...
                if line.get('__group'):
                    col = etree.SubElement(
                        node_line, 'col', para='group', tree='no')
                else:
                    col = etree.SubElement(
                        node_line, 'col', para='yes', tree='no')
