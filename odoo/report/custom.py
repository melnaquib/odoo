...
class report_custom(report_int):
        ...
        for obj in objs:
            ...
            for i in range(len(fields)):
                if not fields[i]:
                    ...
                    if obj:
                        row.append(
                            str(safe_eval('obj.' + fields[i][0], {'obj': obj})))
                    else:
                        row.append(None)
                else:
                    row.append(None)
            ...
            if not levels:
                ...
                for l in key:
                    ...
                    if len(objs):
                        result += self._row_get(cr, uid, objs,
                                                field_new, cond_new, row, group_by)
                    else:
                        result.append(row)
        return result
    ...
    def create(self, cr, uid, ids, datas, context=None):
        ...
        fields = env['ir.report.custom.fields'].browse(
            report['fields_child0']).read()
            ...
            for i in range(4):
                ...
                if field_child:
                    ...
                    if f['fc' + str(i) + '_operande']:
                        ...
                        cond.append((fct[fct_name], f['fc' + str(i) + '_operande']
                                     [1], cond_op, f['fc' + str(i) + '_condition']))
                    else:
                        cond.append(None)
        ...
        if report['field_parent']:
            ...
            def build_tree(obj, level, depth):
                ...
                level.append(depth)
                new_obj = safe_eval(
                    'obj.' + report['field_parent'][1], {'obj': obj})
            ...
            results = self._row_get(
                cr, uid, objs, new_fields, new_cond, group_by=groupby)
        ...
        if groupby is not None:
            ...
            # we use the keys in results since they are ordered, whereas in
            # res_dic.heys() they aren't
            for key in [_f for _f in [x[groupby] for x in results] if _f]:
                row = []
                for col in range(len(fields)):
                    if col == groupby:
                        row.append(fct['groupby']([x[col]
                                                   for x in res_dic[key]]))
                    else:
                        row.append(fct[str(fields[col]['operation'])](
                            [x[col] for x in res_dic[key]]))
                new_res.append(row)
            results = new_res

        if report['type'] == 'table':
            if report['field_parent']:
                res = self._create_tree(
                    uid, ids, report, fields, level, results, context)
            else:
                sort_idx = 0
                ...
                res = self._create_table(
                    uid, ids, report, fields, None, results, context)
            ...
            for r in results:
                ...
                results2.append(row)
            if report['type'] == 'pie':
                res = self._create_pie(
                    cr, uid, ids, report, fields, results2, context)
            elif report['type'] == 'bar':
                res = self._create_bars(
                    cr, uid, ids, report, fields, results2, context)
            elif report['type'] == 'line':
                res = self._create_lines(
                    cr, uid, ids, report, fields, results2, context)
    ...
    def _create_tree(self, uid, ids, report, fields, level, results, context):
        ...
        length = pageSize[0] - 30 - \
            reduce(lambda x, y: x + (y['width'] or 0), fields, 0)
        count = 0
        for f in fields:
            if not f['width']:
                count += 1
        ...
        _append_node('tableSize', '%s' % ','.join(
            ['%.2fmm' % (x['width'],) for x in fields]))
    ...
    def _create_lines(self, cr, uid, ids, report, fields, results, context):
        ...
        fct = {}
        for f in fields:
            field_id = (f['field_child3'] and f['field_child3'][0]) or (f['field_child2'] and f['field_child2'][0]) or (
                f['field_child1'] and f['field_child1'][0]) or (f['field_child0'] and f['field_child0'][0])
            if field_id:
            ...
            for idx in range(len(fields) - 1):
                ...
                for k in keys:
                    ...
                    if fields[idx + 1]['cumulate']:
                        prev += data[k]
                idx0 = 0
                plot = line_plot.T(label=fields[idx + 1]['name'] + ' ' + str(
                    line), data=data_cum, line_style=colors[idx0 * (len(fields) - 1) + idx])
                ar.add_plot(plot)
    ...
    def _create_bars(self, cr, uid, ids, report, fields, results, context):
        ...
        nb_bar = len(data_by_year) * (len(fields) - 1)
        colors = [fill_style.Plain(bgcolor=x)
                  for x in misc.choice_colors(nb_bar)]
        ...
        for line in list(data_by_year.keys()):
            ...
            for idx in range(len(fields) - 1):
                ...
                idx0 = 0
                plot = bar_plot.T(label=fields[idx + 1]['name'] + ' ' + str(line), data=data_cum, cluster=(
                    idx0 * (len(fields) - 1) + idx, nb_bar), fill_style=colors[idx0 * (len(fields) - 1) + idx])
                ar.add_plot(plot)
                abscissa.update(fields_bar[idx])
            idx0 += 1
        abscissa = [[x, None] for x in abscissa]
    ...
    def _create_pie(self, cr, uid, ids, report, fields, results, context):
        ...
        colors = [fill_style.Plain(bgcolor=x)
                  for x in misc.choice_colors(len(results))]
