    ...
    def __init__(self, name, register=True):
        if register:
            ...
            assert name.startswith(
                'report.'), 'Report names should start with "report.".'
        ...
        # TODO the reports have methods with a 'title' kwarg that is redundant
        # with this attribute
        self.title = None
...
class report_rml(report_int):
    ...
    def post_process_xml_data(self, cr, uid, xml, context=None):
        ...
        tmpl_path = get_module_resource(
            'base', 'report', 'corporate_defaults.xml')
        ...
        try:
            ...
            for import_child in stylesheet.findall('./import'):
                if 'href' in import_child.attrib:
                    ...
                    _, imp_file = tools.file_open(
                        imp_file, subdir=xsl_path, pathinfo=True)
        ...
        def translate_aux(doc, lang, t):
            for node in doc:
                ...
                if t:
                    ...
                    if text:
                        text1 = Translation._get_source(
                            self.name2, 'xsl', lang, text)
                        if text1:
                            node.text = node.text.replace(text, text1)
                    if tail:
                        tail1 = Translation._get_source(
                            self.name2, 'xsl', lang, tail)
                        if tail1:
