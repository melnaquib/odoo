...
class _flowable(object):
    ...
    def _tag_table(self, node):
        ...
        if new_node.get('colWidths', False):
            sizes = [utils.unit_get(x)
                     for x in new_node.get('colWidths').split(',')]
...
class _rml_tmpl_draw_string(_rml_tmpl_tag):
    def __init__(self, node, style, localcontext={}):
        ...
        self.pos = [(self.posx, self.posy, align, utils._process_text(
            self, node.text), style.get('td'), style.font_size_get('td'))]
    ...
    def tag_start(self):
        ...
        for (x, y, align, txt, style, fs) in self.pos:
            if align == "left":
                pos2 = len(txt) * fs
                res += "<td width=\'%d\'></td><td style=\'%s\' width=\'%d\'>%s</td>" % (
                    x - posx, style, pos2, txt)
                posx = x + pos2
            if align == "right":
                res += "<td width=\'%d\' align=\'right\' style=\'%s\'>%s</td>" % (
                    x - posx, style, txt)
                posx = x
            if align == "center":
                res += "<td width=\'%d\' align=\'center\' style=\'%s\'>%s</td>" % (
                    (x - posx) * 2, style, txt)
                posx = 2 * x - posx
            i += 1
        res += '</tr></table>'
...
class _rml_tmpl_draw_lines(_rml_tmpl_tag):
    def __init__(self, node, style, localcontext={}):
        self.localcontext = localcontext
        coord = [utils.unit_get(x) for x in utils._process_text(
            self, node.text).split(' ')]
        self.ok = False
        self.posx = coord[0]
        self.posy = coord[1]
...
class _rml_template(object):
    def __init__(self, template, localcontext=None):
        ...
        for pt in template.findall('pageTemplate'):
            ...
            for tmpl in pt.findall('frame'):
                ...
                frames[(posy, posx, tmpl.get('id'))] = _rml_tmpl_frame(
                    posx, utils.unit_get(tmpl.get('width')))
    ...
    def set_next_template(self):
        self.template = self.template_order[(
            self.template_order.index(name) + 1) % self.template_order]
        self.frame_pos = -1
...
'''
        ...
        for story in utils._child_get(self.dom, self, 'story'):
            template = _rml_template(self.dom.findall(
                'template')[0], self.localcontext)
