...
try:
    ...
    def SetCustomFonts(x): return None
    ...
    def __init__(self, nodes, localcontext):
        ...
        for node in nodes:
            for style in node.findall('blockTableStyle'):
                self.table_styles[style.get(
                    'id')] = self._table_style_get(style)
    ...
    def _table_style_get(self, style_node):
        ...
        for node in style_node:
            ...
            if node.tag == 'blockValign':
                ...
                styles.append(
                    ('LEADING', start, stop, utils.unit_get(node.get('length'))))
    ...
    def para_style_get(self, node):
        ...
        if sname:
            if sname in self.styles_obj:
                ...
                _logger.debug(
                    'Warning: style not found, %s - setting default!', node.get('style'))
        if not style:
...
class _rml_doc(object):
    ...
    def docinit(self, els):
        ...
        for node in els:
            ...
            # if registerFontFamily is defined, we register the mapping of the
            # fontName to use for each style.
            for font_family in node.findall('registerFontFamily'):
                ...
                if font_family.get('italic'):
                    addMapping(family_name, 0, 1, font_family.get(
                        'italic').encode('ascii'))
                if font_family.get('bold'):
                    addMapping(family_name, 1, 0, font_family.get(
                        'bold').encode('ascii'))
                if font_family.get('boldItalic'):
                    addMapping(family_name, 1, 1, font_family.get(
                        'boldItalic').encode('ascii'))
    ...
    def render(self, out):
        ...
        if len(el):
            pt_obj = _rml_template(
                self.localcontext, out, el[0], self, images=self.images, path=self.path, title=self.title)
            el = utils._child_get(self.etree, self, 'story')
            pt_obj.render(el)
        else:
            self.canvas = canvas.Canvas(out)
            pd = self.etree.find('pageDrawing')[0]
            pd_obj = _rml_canvas(self.canvas, self.localcontext, None,
...
class _rml_canvas(object):
    ...
    def _drawString(self, node):
        ...
        try:
            ...
            _logger.info(
                "Bad RML: <drawString> tag requires attributes 'x' and 'y'!")
            raise
    ...
    def _rect(self, node):
        if node.get('round'):
            self.canvas.roundRect(radius=utils.unit_get(node.get('round')), **utils.attr_get(
                node, ['x', 'y', 'width', 'height'], {'fill': 'bool', 'stroke': 'bool'}))
        else:
            self.canvas.rect(
    ...
    def _circle(self, node):
        self.canvas.circle(x_cen=utils.unit_get(node.get('x')), y_cen=utils.unit_get(node.get('y')), r=utils.unit_get(
            node.get('radius')), **utils.attr_get(node, [], {'fill': 'bool', 'stroke': 'bool'}))

    def _place(self, node):
        flows = _rml_flowable(self.doc, self.localcontext, images=self.images,
                              path=self.path, title=self.title, canvas=self.canvas).render(node)
...
class _rml_draw(object):
    ...
    def render(self, canvas, doc):
        canvas.saveState()
        cnv = _rml_canvas(canvas, self.localcontext, doc, self.styles,
                          images=self.images, path=self.path, title=self.canvas_title)
...
class _rml_Illustration(platypus.flowables.Flowable):
    ...
    def draw(self):
        drw = _rml_draw(self.localcontext, self.node, self.styles,
                        images=self.self2.images, path=self.self2.path, title=self.self2.title)
...
class _rml_flowable(object):
    ...
    def _textual(self, node):
        ...
        for n in utils._child_get(node, self):
            ...
            if not n.tag == 'bullet':
                if n.tag == 'pageNumber':
                    txt_n.text = self.canvas and str(
                        self.canvas.getPageNumber()) or ''
                else:
    ...
    def _table(self, node):
        ...
        if node.get('colWidths'):
            ...
            colwidths = [utils.unit_get(f.strip())
                         for f in node.get('colWidths').split(',')]
        if node.get('rowHeights'):
            rowheights = [utils.unit_get(f.strip())
    ...
    def _flowable(self, node, extra_style=None):
        ...
        if node.tag == 'para':
            ...
            text_node = self._textual(node).strip().replace(
                '\n\n', '\n').replace('\n', '<br/>')
            instance = platypus.Paragraph(
                text_node, style, **(utils.attr_get(node, [], {'bulletText': 'str'})))
            result = [instance]
            if LooseVersion(reportlab.Version) > LooseVersion('3.0') and not instance.getPlainText().strip() and instance.text.strip():
                result.append(platypus.Paragraph(
                    '&nbsp;<br/>', style, **(utils.attr_get(node, [], {'bulletText': 'str'}))))
            return result
            ...
            args = utils.attr_get(node, [], {'ratio': 'float', 'xdim': 'unit', 'height': 'unit', 'checksum': 'int',
                                             'quiet': 'int', 'width': 'unit', 'stop': 'bool', 'bearers': 'int', 'barWidth': 'float', 'barHeight': 'float'})
            codes = {
...
class _rml_template(object):
    def __init__(self, localcontext, out, node, doc, images=None, path='.', title=None):
        ...
        if self.localcontext.get('company'):
            pageSize = pagesize_map.get(
                self.localcontext.get('company').rml_paper_format, A4)
        if node.get('pageSize'):
            ps = [x.strip() for x in node.get('pageSize').replace(')',
                                                                  '').replace('(', '').split(',')]
            pageSize = (utils.unit_get(ps[0]), utils.unit_get(ps[1]))

        self.doc_tmpl = TinyDocTemplate(out, pagesize=pageSize, **utils.attr_get(node, ['leftMargin', 'rightMargin', 'topMargin', 'bottomMargin'], {
                                        'allowSplitting': 'int', 'showBoundary': 'bool', 'rotation': 'int', 'title': 'str', 'author': 'str'}))
        self.page_templates = []
        self.styles = doc.styles
        self.doc = doc
        self.image = []
        ...
        for pt in pts:
            ...
            for frame_el in pt.findall('frame'):
                frame = platypus.Frame(**(utils.attr_get(frame_el, ['x1', 'y1', 'width', 'height', 'leftPadding',
                                                                    'rightPadding', 'bottomPadding', 'topPadding'], {'id': 'str', 'showBoundary': 'bool'})))
            ...
            if len(gr):
                ...
                drw = _rml_draw(
                    self.localcontext, gr[0], self.doc, images=images, path=self.path, title=self.title)
                self.page_templates.append(platypus.PageTemplate(
                    frames=frames, onPage=drw.render, **utils.attr_get(pt, [], {'id': 'str'})))
            else:
                drw = _rml_draw(self.localcontext, node,
    ...
    def render(self, node_stories):
        ...
        fis = []
        r = _rml_flowable(self.doc, self.localcontext, images=self.images,
                          path=self.path, title=self.title, canvas=None)
        story_cnt = 0
...
def parseNode(rml, localcontext=None, fout=None, images=None, path='.', title=None):
    ...
    try:
        ...
        _logger.info('Cannot set font mapping: %s', "".join(
