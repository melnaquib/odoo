            ...
            for ctx in safe_eval(n.get('rml_loop'), {}, self.localcontext):
                ...
                if (tagname is None) or (n.tag == tagname):
                    if n.get('rml_except', False):
                        try:
                            safe_eval(n.get('rml_except'),
                                      {}, self.localcontext)
                        except GeneratorExit:
                            continue
                        except Exception as e:
                            _logger.info('rml_except: "%s"', n.get(
                                'rml_except', ''), exc_info=True)
                            continue
                    if n.get('rml_tag'):
                        try:
                            (tag, attr) = safe_eval(
                                n.get('rml_tag'), {}, self.localcontext)
                            n2 = copy.deepcopy(n)
                            n2.tag = tag
                            ...
                            _logger.info('rml_tag: "%s"', n.get(
                                'rml_tag', ''), exc_info=True)
                            yield n
                    else:
                        yield n
            continue
...
def _process_text(self, txt):
    ...
    while sps:
        ...
        result += tools.ustr(self.localcontext.get('translate',
                                                   lambda x: x)(to_translate))
        if sps:
            txt = None
            try:
                expr = sps.pop(0)
                ...
                _logger.info(
                    "Failed to evaluate expression [[ %s ]] with context %r while rendering report, ignored.", expr, self.localcontext)
