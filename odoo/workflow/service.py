...
class WorkflowService(object):
    ...
    def trigger(self):
        self.cr.execute('select instance_id from wkf_triggers where res_id=%s and model=%s',
                        (self.record.id, self.record.model))
        res = self.cr.fetchall()
        for (instance_id,) in res:
            self.cr.execute(
                'select %s,res_type,res_id from wkf_instance where id=%s', (self.session.uid, instance_id,))
    ...
    def create(self):
        ...
        if not wkf_ids:
            self.cr.execute(
                'select id from wkf where osv=%s and on_create=True', (self.record.model,))
    ...
    def validate(self, signal):
        result = False
        # ids of all active workflow instances for a corresponding resource
        # (id, model_nam)
        self.cr.execute('select id from wkf_instance where res_id=%s and res_type=%s and state=%s',
        ...
        for (instance_id,) in self.cr.fetchall():
            wi = WorkflowInstance(
                self.session, self.record, {'id': instance_id})
    ...
    def redirect(self, new_rid):
        ...
        self.cr.execute('select id, wkf_id from wkf_instance where res_id=%s and res_type=%s',
                        (self.record.id, self.record.model))
        ...
        for old_inst_id, workflow_id in self.cr.fetchall():
            ...
            if new_id:
                ...
                self.cr.execute(
                    'select id from wkf_workitem where subflow_id=%s', (old_inst_id,))
                for (item_id,) in self.cr.fetchall():
                    # redirect all those workitems to the wkf instance of the
                    # new resource
                    self.cr.execute(
                        'update wkf_workitem set subflow_id=%s where id=%s', (new_id[0], item_id))
